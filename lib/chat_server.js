    ...
    var conversations = JSON.parse(JSON.stringify(findConversationsForUser(username)));
		...
		if (conversations.length == 0) {
			for (var index in usernames) {
				conversations.push({"members": [username, usernames[index]], "messageList": []});
			}
			for (var index in savedUsernames) {
				if (usernames.hasOwnProperty(savedUsernames[index])) {
					conversations.push({"members": [username, savedUsernames[index]], "messageList": []});
				}
			}
