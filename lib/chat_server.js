...
function handleMessageUsertoUser(socket) {
  socket.on('chatMessage', function(messageInfo) {
    updateOneToOneConversation(messageInfo.to, messageInfo.from, JSON.parse(JSON.stringify(findConversationsForUser(messageInfo.messageList))));
    console.log('CHATMESSAGE EVENT')
    console.log(messageInfo.messageList);
    console.log("-----------------------------------------------")

    for (var index in messageInfo.messageList) {
      var message = messageInfo.messageList[index];
    ...
    if (usernames[messageInfo.to] !== undefined) {
      console.log("SENDING MESSAGE TO " + messageInfo.to);
