  ...
  socket.on('loadConversations', function(username) {
    ...
    var conversations = JSON.parse(JSON.stringify(findConversationsForUser(username)));
		...
		if (conversations.length == 0) {
			...
			conversations.push({"members": [username, "No Conversation Selected"], "messageList": ["No Conversation Selected": "Please go to your friends list an open a chat with a friend!"]});
